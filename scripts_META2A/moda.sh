...
function moda_ou_mediana(a) {
        # retorna sempre o modo 
        # caso haja empate, retorna o de maior rssi
        # caso nao exista moda, retorna a mediana
        lengtha=asort(a)
        delete h
        delete b
        for (i=lengtha; i>0;i--) b[lengtha-i+1]=a[i]
        v=0
        m=0
        i=1
        while (i <= lengtha) {
        ...
        mediana=a[int(lengtha/2)+1]
        if (m > 1) return v
        else return mediana     
}

function calcula_moda(lista_de_amostras,win) {
	delete amostras
	delete amostras_janela
	split(lista_de_amostras,amostras,"#")
	# se a janela nao foi preenchida ainda, retorna a moda das ultimas amostras
	...
	for (i=length(amostras); (i > p && i > 0) ; i--){
		...
		if (debug == 1) printf ("%d,",amostras_janela[j])
		j=j+1
		if (debug == 1) print "amostra: "amostras[i]" j: "j
	}
	unew=moda_ou_mediana(amostras_janela)
	if (debug == 1) print"\n"unew
	if (debug == 1) print "win: "win" pmedio: "ponto_medio" nova moda: " unew
	return unew
}
...
function update_array_atual(mac) {
	...
	for (i in array_old) {
		...
		if ( array_old[i][3] == mac ) { #atualiza
			...
			array_atual[mac][5]=calcula_moda(array_atual[mac][10],janela)  # RSSI moda considerando amostras antigas na janela + a atual
...
function moda {
	...
	scan_file_out="saidas/$pcapfile/moda_"$janela"_offset_"$offset"_scan_file_tratado.dat"
	if [ ! -d saidas/$pcapfile/ ]; then mkdir -p saidas/$pcapfile/; fi
	if [ ! -d resultados/$pcapfile/ ]; then mkdir -p resultados/$pcapfile/; fi
	scan_moda "$scan_file_out"

	bhisterese="on"
	moda_result="resultados/$pcapfile/moda_"$janela"_offset_"$offset"_histerese_"$bhisterese"_result.dat"
	wpa_handoff_function "$scan_file_out" "$moda_result"
	gzip -f -9 $moda_result
	
	bhisterese="off"
	moda_result="resultados/$pcapfile/moda_"$janela"_offset_"$offset"_histerese_"$bhisterese"_result.dat"
	another_handoff_function "$scan_file_out" "$moda_result"
	gzip -f -9 $moda_result
