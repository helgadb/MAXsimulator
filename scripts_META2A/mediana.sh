...
function calcula_mediana(lista_de_amostras,win) {
	delete amostras
	delete amostras_janela
	split(lista_de_amostras,amostras,"#")
	# se a janela nao foi preenchida ainda, retorna a mediana das ultimas amostras
	...
	ponto_medio = int(win/2)+1
	unew=amostras_janela[ponto_medio]
	if (debug == 1) print "win: "win" pmedio: "ponto_medio" nova mediana: " unew
	return unew
}
...
function update_array_atual(mac) {
	...
	for (i in array_old) {
		...
		if ( array_old[i][3] == mac ) { #atualiza
			...
			array_atual[mac][5]=calcula_mediana(array_atual[mac][10],janela)  # RSSI mediana considerando amostras antigas na janela + a atual
...
function mediana {
	...
	scan_file_out="saidas/$pcapfile/mediana_"$janela"_offset_"$offset"_scan_file_tratado.dat"
	if [ ! -d saidas/$pcapfile/ ]; then mkdir -p saidas/$pcapfile/; fi
	if [ ! -d resultados/$pcapfile/ ]; then mkdir -p resultados/$pcapfile/; fi
	scan_mediana "$scan_file_out"

	bhisterese="on"
	mediana_result="resultados/$pcapfile/mediana_"$janela"_offset_"$offset"_histerese_"$bhisterese"_result.dat"
	wpa_handoff_function "$scan_file_out" "$mediana_result"
	gzip -f -9 $mediana_result

	bhisterese="off"
	mediana_result="resultados/$pcapfile/mediana_"$janela"_offset_"$offset"_histerese_"$bhisterese"_result.dat"
	another_handoff_function "$scan_file_out" "$mediana_result"
	gzip -f -9 $mediana_result
