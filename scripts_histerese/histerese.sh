function histerese_handoff_function {
	file_scan="$1"
	saida="$2"
	margem_de_histerese="$3"
	if [ -f $saida ]; then rm $saida; fi
	cat $file_scan | awk -v saida="$saida" -v margem_de_histerese="$margem_de_histerese" 'BEGIN{c=0;bfound=0;bfound2=0;ap_pretendente_info="";ap_atual_scan=""}
...
histerese_result=""
function histerese {
	margem_de_histerese="$1"
	scan_file_out="saidas/$pcapfile/histerese_"$margem_de_histerese"_offset_"$offset"_scan_file_tratado.dat"
	histerese_result="resultados/$pcapfile/histerese_"$margem_de_histerese"_offset_"$offset"_histerese_on_result.dat"
	...
	histerese_handoff_function "$scan_file_out" "$histerese_result" "$margem_de_histerese"
	gzip -f -9 $scan_file_out
	gzip -f -9 $histerese_result
}
